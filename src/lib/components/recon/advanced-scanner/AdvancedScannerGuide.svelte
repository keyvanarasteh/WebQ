    ...
    >
        ...
        >
            ...
            <div class="relative bg-gradient-to-r from-teal-950/40 via-indigo-900/10 to-transparent p-6 border-b border-teal-500/10 shrink-0">
                ...
                <button 
                    onclick={close}
                    class="absolute top-4 right-4 p-2 text-zinc-400 hover:text-white bg-white/5 hover:bg-teal-500/20 rounded-xl transition-all border border-white/5 hover:border-teal-500/30 font-medium"
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative z-10">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-2xl font-bold tracking-tight text-white">{m.recon_scanner_guide_title ? m.recon_scanner_guide_title() : 'Advanced Deep Scanning'}</h2>
                            <p class="text-sm text-zinc-400 mt-1 max-w-md">{m.recon_scanner_subtitle ? m.recon_scanner_subtitle() : 'Automated path fuzzing, fingerprinting, and surface discovery'}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex bg-black/40 p-1 rounded-lg border border-white/5 mt-4 sm:mt-0">
                        {#each tabs as tab (tab.id)}
                            <button
                                ...
                                class="flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-all {activeTab === tab.id ? 'bg-teal-500/20 text-teal-300 border border-teal-500/20 shadow-sm' : 'text-zinc-500 hover:text-zinc-300 hover:bg-white/5 border border-transparent'}"
                            >
                                <tab.icon size={14} />
            ...
            <div class="flex-1 overflow-y-auto p-6 relative">
                {#if activeTab === 0}
                    <div transition:fade={{duration: 200}}>
                        ...
                        <p class="text-sm text-zinc-400 mb-6 max-w-3xl">Advanced scanners begin by stripping the target bare. They analyze HTTP headers, injected cookies, and unique DOM signatures to identify the exact technology stack running beneath.</p>
                        ...
                        <div class="relative bg-[#0d1117] rounded-xl border border-teal-500/30 shadow-[0_0_30px_rgba(20,184,166,0.1)] overflow-hidden font-mono text-sm mb-6 flex flex-col md:flex-row">
                            <div class="bg-zinc-900/50 p-4 border-b md:border-b-0 md:border-r border-teal-500/20 w-full md:w-1/3 flex flex-col justify-center">
                                ...
                                <p class="text-xs text-zinc-500 italic mb-4">Analyzing response anomalies...</p>
                                <div class="space-y-1 text-xs">
                                    <p class="text-orange-300">HTTP/2 200 OK</p>
                                    <p class="text-zinc-300">Server: <span class="text-red-400 font-bold bg-red-400/10 px-1 rounded">cloudflare</span></p>
                                    <p class="text-zinc-300">X-Powered-By: <span class="text-red-400 font-bold bg-red-400/10 px-1 rounded">PHP/8.1.2</span></p>
                                    <p class="text-zinc-300">Set-Cookie: <span class="text-red-400 font-bold bg-red-400/10 px-1 rounded">PHPSESSID</span>=abc123</p>
                                </div>
                            </div>
                                ...
                                <div class="mt-4 space-y-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-zinc-500">Cloudflare edge routing identified.</p>
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-zinc-500">PHP 8.1.2 (Potentially vulnerable to deserialization)</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                {:else if activeTab === 1}
                    <div transition:fade={{duration: 200}}>
                        ...
                        <p class="text-sm text-zinc-400 mb-6 max-w-3xl">Attackers don't navigate websites; they brute-force them. By leveraging massive wordlists (like SecLists), scanners discover hidden administrative panels, `.git` exposures, and unlinked backup files.</p>
                        ...
                        <div class="bg-[#0d1117] rounded-xl border border-teal-500/10 shadow-2xl overflow-hidden font-mono text-xs sm:text-sm shadow-[0_0_30px_rgba(0,0,0,0.5)]">
                            <div class="bg-zinc-900/80 px-4 py-3 flex items-center justify-between border-b border-white/5 backdrop-blur">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="ml-2 text-zinc-500 text-xs tracking-wider">ffuf / gobuster simulation</span>
                                </div>
                            ...
                            <div class="p-5 space-y-2 text-zinc-300 h-72 overflow-y-auto custom-scrollbar">
                                ...
                                <p class="text-zinc-500 mb-4">________________________________________________</p>
                                
                                <div class="flex items-center gap-4 text-emerald-400"><span class="w-16">200 OK</span> <span class="text-zinc-300">/about</span> <span class="text-zinc-600 text-xs">(Size: 4502)</span></div>
                                <div class="flex items-center gap-4 text-emerald-400"><span class="w-16">200 OK</span> <span class="text-zinc-300">/contact</span> <span class="text-zinc-600 text-xs">(Size: 3102)</span></div>
                                <div class="flex items-center gap-4 text-orange-400"><span class="w-16">301 MOVD</span> <span class="text-zinc-300">/admin</span> <span class="text-zinc-600 text-xs">-> /admin/login.php</span></div>
                                <div class="flex items-center gap-4 text-orange-400"><span class="w-16">403 FORB</span> <span class="text-zinc-300">/api/v1</span> <span class="text-zinc-600 text-xs">(Size: 152)</span></div>
                                <div class="flex items-center gap-4 text-orange-400"><span class="w-16">301 MOVD</span> <span class="text-zinc-300">/.git</span> <span class="text-zinc-600 text-xs bg-red-500/20 text-red-400 px-1 rounded ml-2">HIGH RISK!</span></div>
                                <div class="flex items-center gap-4 text-emerald-400"><span class="w-16">200 OK</span> <span class="text-zinc-300">/backup.zip</span> <span class="text-zinc-600 text-xs bg-red-500/20 text-red-400 px-1 rounded ml-2">SOURCE EXFILTRATION!</span></div>
                                
                                <p class="text-zinc-500 mt-4 border-t border-white/5 pt-2">:: Progress: [4614/4614] :: Job [1/1] :: 450 req/sec ::</p>
                            </div>
                        </div>
                    </div>
                {:else if activeTab === 2}
                    <div transition:fade={{duration: 200}}>
                        ...
                        <p class="text-sm text-zinc-400 mb-6 max-w-3xl">Modern automated scanners employ YAML-based templates (like Nuclei) to spray massive payloads identifying 1-day vulnerabilities instantly across thousands of subdomains.</p>
            ...
            <div class="bg-[#0A0C10] p-4 border-t border-teal-500/10 flex items-center justify-between shrink-0 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.5)] z-20">
                ...
                <div class="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                    ...
                    {#if activeTab < tabs.length - 1}
                        <button 
                            onclick={nextTab}
                            class="flex items-center gap-1.5 px-6 py-2.5 bg-teal-600 hover:bg-teal-500 text-white text-sm font-semibold rounded-xl shadow-[0_0_20px_rgba(20,184,166,0.4)] transition-all transform hover:scale-105 active:scale-95 focus:ring-2 focus:ring-teal-500/50 outline-none"
                        >
                    ...
                    {:else}
                        <button 
                            onclick={close}
                            class="flex items-center gap-1.5 px-8 py-2.5 bg-indigo-500 hover:bg-indigo-400 text-white text-sm font-bold rounded-xl shadow-[0_0_20px_rgba(99,102,241,0.4)] transition-all transform hover:scale-105 active:scale-95 focus:ring-2 focus:ring-indigo-500/50 outline-none"
                        >
                            Complete
                        </button>
                    {/if}
