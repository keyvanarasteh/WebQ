...
{#if isOpen}
	...
	>
		...
		<div
			class="relative w-full max-w-4xl bg-qix-obsidian border border-[#27272a] rounded-xl shadow-2xl overflow-hidden flex flex-col md:flex-row shadow-emerald-900/20"
			transition:fly={{ y: 20, duration: 400, easing: backOut }}
		>
			<!-- Close Button -->
			<button
				onclick={() => (isOpen = false)}
				class="absolute top-4 right-4 p-2 text-gray-400 hover:text-white bg-[#121214] hover:bg-[#27272a] rounded-full transition-colors z-10 focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
				aria-label="Close"
			>
				<X class="size-4" />
			</button>
			...
			<div class="w-full md:w-64 bg-[#121214] border-r border-[#27272a] shrink-0 flex flex-col">
				<div class="p-6 border-b border-[#27272a]">
					<div class="flex items-center gap-3">
						...
						<div>
							<h2 id="security-headers-guide-title" class="font-bold text-white text-sm">Security Headers</h2>
				...
				<div class="p-4 flex-1 flex flex-row md:flex-col gap-2 overflow-x-auto md:overflow-visible">
					{#each tabs as tab (tab.id)}
						...
						>
							<tab.icon class="size-4 {activeTab === tab.id ? 'text-emerald-400' : 'text-gray-500'}" />
                ...
                <div class="p-6 border-t border-[#27272a] hidden md:block">
					<div class="bg-qix-obsidian rounded p-3 border border-[#27272a]">
						<div class="text-[10px] font-mono text-gray-500 uppercase tracking-wider mb-1">Module Standard</div>
						<div class="text-xs text-gray-300 font-medium">OWASP Top 10</div>
					</div>
				</div>
			</div>
				...
				{#if activeTab === 'headers'}
					<div in:fade={{ duration: 200, delay: 100 }}>
						...
						<div class="space-y-4">
                            ...
                            <div class="bg-[#121214] border border-[#27272a] p-4 rounded-xl">
                                ...
                                <div class="bg-qix-obsidian p-3 rounded-lg border border-[#27272a] overflow-x-auto custom-scrollbar">
                                    <code class="text-xs font-mono text-gray-300 whitespace-nowrap">Strict-Transport-Security: max-age=31536000; includeSubDomains; preload</code>
                                </div>
                            </div>
                            ...
                            <div class="bg-[#121214] border border-[#27272a] p-4 rounded-xl">
                                ...
                                <div class="bg-qix-obsidian p-3 rounded-lg border border-[#27272a] overflow-x-auto custom-scrollbar">
                                    <code class="text-xs font-mono text-gray-300 whitespace-nowrap">Content-Security-Policy: default-src 'self'; script-src 'self' https://apis.google.com</code>
                                </div>
                            </div>
                            ...
                            <div class="bg-[#121214] border border-[#27272a] p-4 rounded-xl">
                                ...
                                <p class="text-xs text-gray-400 mb-3">Prevents Clickjacking attacks by denying browsers from rendering the page inside an invisible <code>&lt;iframe&gt;</code> on an attacker's website.</p>
                                <div class="bg-qix-obsidian p-3 rounded-lg border border-[#27272a] overflow-x-auto custom-scrollbar">
