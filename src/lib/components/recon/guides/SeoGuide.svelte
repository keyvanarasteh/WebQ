...
{#if isOpen}
	...
	>
		...
		<div
			class="relative w-full max-w-4xl bg-background border border-base rounded-xl shadow-2xl overflow-hidden flex flex-col md:flex-row shadow-cyan-900/20"
			...
			<div class="w-full md:w-64 bg-[#121214] border-r border-base shrink-0 flex flex-col">
				<div class="p-6 border-b border-base">
					<div class="flex items-center gap-3">
						...
						<div>
							<h2 id="seo-guide-title" class="font-bold text-primary-text text-sm">SEO & Metadata</h2>
							<div class="text-[10px] text-accent/70 uppercase tracking-widest font-mono mt-1">Reconnaissance</div>
						</div>
					</div>
				</div>
				...
				<div class="p-4 flex-1 flex flex-row md:flex-col gap-2 overflow-x-auto md:overflow-visible">
					{#each tabs as tab (tab.id)}
						<button
                            ...
                            {activeTab === tab.id
								? 'bg-cyan-500/10 text-accent border border-cyan-500/20'
								: 'text-muted hover:bg-surface hover:text-primary-text border border-transparent'}"
						>
							<tab.icon class="size-4 {activeTab === tab.id ? 'text-accent' : 'text-muted'}" />
                ...
                <div class="p-6 border-t border-base hidden md:block">
					<div class="bg-background rounded p-3 border border-base">
						<div class="text-[10px] font-mono text-muted uppercase tracking-wider mb-1">Module Type</div>
						<div class="text-xs text-primary-text font-medium">OSINT Footprinting</div>
					</div>
				</div>
			</div>
			...
			<div class="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-8 max-h-[80vh] md:max-h-[600px] bg-background">
				...
				{#if activeTab === 'intro'}
					<div in:fade={{ duration: 200, delay: 100 }}>
						<h3 class="text-xl font-bold text-primary-text mb-4">Search Engine & Metadata Recon</h3>
						<p class="text-sm leading-relaxed text-muted mb-6">
							While typically the domain of marketing teams, <strong class="text-primary-text">SEO (Search Engine Optimization) and Metadata</strong> attributes hold massive implications for OSINT (Open-Source Intelligence) and footprinting. Attackers extract software versions, authors, indexing directives, and social network integrations from meta tags.
						</p>
						...
						<div class="grid grid-cols-1 gap-4">
                            <div class="p-5 bg-[#121214] border border-base rounded-xl flex items-start gap-4">
								...
								<div>
									<h4 class="text-sm font-bold text-primary-text mb-1">Open-Source Intelligence</h4>
							...
							<div class="p-5 bg-[#121214] border border-base rounded-xl flex items-start gap-4">
								...
								<div>
									<h4 class="text-sm font-bold text-primary-text mb-1">Information Disclosure</h4>
									<p class="text-xs text-muted leading-relaxed">
				...
				{#if activeTab === 'threats'}
					<div in:fade={{ duration: 200, delay: 100 }}>
						<h3 class="text-xl font-bold text-primary-text mb-2 flex items-center gap-2">
                            <ShieldAlert class="size-5 text-accent" /> Metadata Threat Vectors
                        </h3>
				...
				{#if activeTab === 'osint'}
					<div in:fade={{ duration: 200, delay: 100 }}>
						<h3 class="text-xl font-bold text-primary-text mb-2 flex items-center gap-2">
							<Globe class="size-5 text-accent" /> OSINT & Open Graph Metrics
						</h3>
						<p class="text-sm text-muted mb-6">
							Open Graph (<code class="text-accent bg-cyan-950/30 px-1 py-0.5 rounded">og:title</code>, <code class="text-accent bg-cyan-950/30 px-1 py-0.5 rounded">og:image</code>) and Twitter Cards provide structured data for link unfurling.
						</p>
