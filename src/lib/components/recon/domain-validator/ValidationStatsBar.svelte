...
<div class="bg-background/5 bg-background border border-base rounded-xl p-6 shadow-sm mb-6">
  ...
  {#if isLoading && !stats}
      <div class="w-full h-8 bg-surface rounded-full animate-pulse mt-4"></div>
  {:else if stats}
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div class="text-center p-3 bg-background border border-base rounded-lg">
          ...
          <div class="text-center p-3 bg-green-50/50 border border-green-500/20 rounded-lg">
              <p class="text-xs text-green-600 font-black tracking-widest mb-1">{m.val_valid()}</p>
              <p class="text-2xl font-black text-green-600">{stats.valid}</p>
          </div>
          <div class="text-center p-3 bg-red-50/50 border border-red-500/20 rounded-lg">
              <p class="text-xs text-red-600 font-black tracking-widest mb-1">{m.val_invalid()}</p>
          ...
          <div class="text-center p-3 bg-background border border-base rounded-lg">
              ...
              <p class="text-2xl font-black text-primary-text text-muted">{stats.skipped}</p>
          </div>
      </div>
      
      <div class="w-full h-4 bg-surface rounded-full overflow-hidden flex border border-base shadow-inner">
