  ...
  {:else if data}
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {#each [
            ...
            <div class="p-4 bg-background border border-base hover:border-base transition-colors rounded-lg flex flex-col justify-between h-full">
                <div class="mb-4">
                ...
                <div>
                    {#if header.val}
                        <span class="inline-flex items-center px-2 py-1 bg-green-500/10 text-green-500 text-xs font-black rounded border border-green-500/20 uppercase tracking-widest">{m.sec_header_configured()}</span>
                    {:else}
