...
<div class="overflow-x-auto w-full rounded-lg border border-slate-700/50">
	<table class="w-full text-left text-sm text-slate-300">
		<thead class="bg-background text-xs uppercase text-muted font-fira border-b border-slate-700/50">
			<tr>
		...
		<tbody class="divide-y divide-slate-700/50 font-mono bg-slate-900/50">
			{#each data as item, i (item)}
				<tr class="hover:bg-indigo-500/10 transition-colors group" in:fade={{ delay: Math.min(i * 5, 500), duration: 200 }}>
					<td class="px-4 py-3 whitespace-nowrap text-muted border-r border-slate-700/50 text-xs">
						{i + 1}
					</td>
					...
					<td class="px-6 py-3 whitespace-nowrap border-r border-slate-700/50 text-center">
						<span class="inline-flex items-center gap-1.5 rounded bg-background px-2 py-1.5 text-xs text-muted border border-slate-700/50 group-hover:border-indigo-500/50 transition-colors">
