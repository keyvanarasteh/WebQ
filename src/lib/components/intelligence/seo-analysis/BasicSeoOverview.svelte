...
<div class="bg-white/5 bg-background border border-base border-base rounded-xl p-6 shadow-sm h-full">
  <SeoGuide bind:isOpen={isGuideOpen} />
  
  <div class="flex items-center justify-between mb-4">
      <h3 class="text-lg font-bold text-inverse text-accent">{m.seo_basic_meta_data()}</h3>
  ...
  {#if isLoading}
    <div class="space-y-4 animate-pulse">
        <div class="h-10 bg-surface bg-surface rounded"></div>
        <div class="h-16 bg-surface bg-surface rounded"></div>
    </div>
  {:else if data}
    <div class="grid grid-cols-1 gap-4">
        <!-- Title -->
        <div class="p-3 bg-background dark:bg-[#121214] rounded-lg border border-base border-base">
        ...
        <!-- H1 Tags -->
        <div class="p-3 bg-background dark:bg-[#121214] rounded-lg border border-base border-base flex justify-between items-center">
            <div>
