...
{#if isOpen}
    ...
    <div class="fixed inset-0 z-50 flex items-start justify-center pt-32 bg-black/60 backdrop-blur-sm" transition:fade={{ duration: 150 }} onclick={() => isOpen = false}>
        ...
        <div class="w-full max-w-2xl bg-qix-obsidian border border-cyan-500/30 rounded-xl shadow-[0_0_50px_-12px_rgba(34,211,238,0.2)] overflow-hidden flex flex-col" transition:fly={{ y: -20, duration: 250, easing: backOut }} onclick={(e) => e.stopPropagation()}>
            <div class="flex items-center px-4 py-3 border-b border-[#27272a] gap-3">
                <Terminal class="text-cyan-500 size-5" />
                <!-- svelte-ignore a11y_autofocus -->
                <input 
                    type="text" 
                    bind:value={searchQuery}
                    placeholder="Type a command or search..."
                    class="w-full bg-transparent border-none text-white focus:outline-none focus:ring-0 text-sm font-mono"
                    autofocus
                />
                <button onclick={() => isOpen = false} class="text-zinc-500 hover:text-white transition-colors bg-white/5 hover:bg-white/10 rounded-md p-1 border border-transparent hover:border-white/10">
                    <X size={16} />
                </button>
            </div>
            ...
            <div class="max-h-96 overflow-y-auto p-2 scrollbar-thin">
                {#if filteredCommands.length === 0}
                    <div class="px-4 py-8 text-center text-zinc-500 font-mono text-sm">No commands found matching "{searchQuery}"</div>
                {:else}
                    {#each filteredCommands as cmd (cmd.id)}
                        <button 
                            onclick={() => executeCommand(cmd.path)}
                            class="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left text-zinc-300 hover:bg-cyan-500/10 hover:text-cyan-400 transition-colors border border-transparent hover:border-cyan-500/30 group"
                        >
                            ...
                            <div class="flex flex-col">
                                ...
                                <span class="text-[10px] text-zinc-500 font-mono uppercase tracking-widest">{cmd.category}</span>
                            </div>
                        </button>
                    {/each}
                {/if}
            ...
            <div class="bg-black/40 px-4 py-2 border-t border-[#27272a] text-xs text-zinc-500 flex justify-between items-center font-mono">
