<script lang="ts">
    ...
    import { X, Search, Link, ShieldCheck, AlertTriangle, CloudOff, Code2, Server, ArrowRight, ChevronRight, ChevronLeft, Globe } from 'lucide-svelte';
...
{#if isOpen}
    ...
    >
        ...
        >
            <!-- Header Area -->
            <div class="relative bg-linear-to-r from-amber-950/40 via-orange-900/10 to-transparent p-6 border-b border-amber-500/10 shrink-0">
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative z-10">
                    <div class="flex items-center gap-4">
                        <div class="w-14 h-14 rounded-2xl bg-linear-to-br from-amber-500/20 to-orange-600/10 flex items-center justify-center border border-amber-500/30 shadow-inner">
                            <Link size={28} class="text-amber-400" />
                        </div>
                        <div>
                            <h2 class="text-2xl font-bold tracking-tight text-primary-text">{m.sec_takeover_guide_title ? m.sec_takeover_guide_title() : 'Subdomain Takeover Anatomy'}</h2>
                            <p class="text-sm text-muted mt-1 max-w-md">{m.sec_takeover_guide_desc ? m.sec_takeover_guide_desc() : 'Learn how dangling DNS records lead to complete subdomain hijacking.'}</p>
                        </div>
                    </div>
            ...
            <div class="flex-1 overflow-y-auto p-6 relative">
                                ...
                                <div class="flex items-center justify-between w-full mb-6 relative py-4">
                                    ...
                                    <div class="flex flex-col items-center z-10 w-24">
                                        <div class="relative w-12 h-12 rounded-lg bg-red-950 border-2 border-red-500 flex items-center justify-center mb-2">
                                            ...
                                            <div class="absolute -right-2 -top-2 bg-red-600 text-primary-text min-w-5 h-5 rounded flex items-center justify-center font-bold text-[10px] px-1">404</div>
                                        </div>
                ...
                {:else if activeTab === 1}
                    <div transition:fade={{duration: 200}}>
                        ...
                        <p class="text-sm text-muted mb-4 max-w-3xl">An attacker discovers the dangling record and claims the target resource on the third-party service provider, effectively gaining full control over content served at the victim's subdomain.</p>
...
&#125;
<span class="text-muted"># Attacker now fully controls auth.target.com! They can serve malware, collect credentials, or read cookies.</span>
</pre>
                                </div>
                            </div>
                        </div>
                ...
                {:else if activeTab === 2}
                    <div transition:fade={{duration: 200}}>
                        ...
                        <p class="text-sm text-muted mb-6 max-w-3xl">Subdomain takeovers are prevented by strict lifecycle management of infrastructure and continuous scanning of DNS zones against active assets.</p>
...
<span class="text-muted italic"># Instead of manual provisioning, use Infrastructure as Code (IaC)</span>
<span class="text-muted italic"># which tracks dependencies and ensures DNS is deleted before</span>
<span class="text-muted italic"># the cloud resource is destroyed.</span>
...
<span class="text-purple-400">resource</span> <span class="text-green-300">"aws_route53_record"</span> <span class="text-blue-400">"auth_cname"</span> &#123;
  ...
  <span class="text-muted italic"># ✅ Explicit dependency ensures safe teardown order:</span>
