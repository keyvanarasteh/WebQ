    ...
    >
        ...
        >
            ...
            <div class="relative bg-gradient-to-r from-red-950/40 via-rose-900/10 to-transparent p-6 border-b border-red-500/10 shrink-0">
                ...
                <button 
                    onclick={close}
                    class="absolute top-4 right-4 p-2 text-zinc-400 hover:text-white bg-white/5 hover:bg-red-500/20 rounded-xl transition-all border border-white/5 hover:border-red-500/30 font-medium"
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative z-10">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h2 class="text-2xl font-bold tracking-tight text-white">{m.sec_nmap_title ? m.sec_nmap_title() : 'Port Scanning & Zero-Day Correlation'}</h2>
                            <p class="text-sm text-zinc-400 mt-1 max-w-md">{m.sec_nmap_guide_mechanics ? m.sec_nmap_guide_mechanics() : 'Probing networks to identify services and mapping them to known CVEs.'}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex bg-black/40 p-1 rounded-lg border border-white/5 mt-4 sm:mt-0">
                        {#each tabs as tab (tab.id)}
                            <button
                                ...
                                class="flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-all {activeTab === tab.id ? 'bg-red-500/20 text-red-300 border border-red-500/20 shadow-sm' : 'text-zinc-500 hover:text-zinc-300 hover:bg-white/5 border border-transparent'}"
                            >
                                <tab.icon size={14} />
            ...
            <div class="flex-1 overflow-y-auto p-6 relative">
                            ...
                            <div class="bg-black/50 border border-white/5 rounded-xl p-6 flex flex-col items-center justify-center relative shadow-inner h-full">
                                <div class="flex items-center justify-between w-full mb-6 relative py-4">
                                    <div class="flex flex-col items-center z-10 w-20">
                                        <div class="w-12 h-12 rounded-full bg-zinc-800 border-2 border-zinc-600 flex items-center justify-center shadow-lg">
                                            <Search size={20} class="text-zinc-400"/>
                                        </div>
                                        <span class="text-xs text-zinc-400 mt-2">Scanner</span>
                                    </div>
                                    ...
                                    <div class="flex flex-col items-center z-10 w-20">
                                        <div class="w-14 h-14 rounded-lg bg-zinc-900 border-2 border-zinc-700 flex flex-col items-center justify-center text-zinc-300 shadow-xl">
                ...
                {:else if activeTab === 1}
                    <div transition:fade={{duration: 200}}>
                        ...
                        <p class="text-sm text-zinc-400 mb-6 max-w-3xl">Once open ports are found, scanners interact with the services using hardcoded probes to extract precise software versions, which are then cross-referenced against vulnerability databases (NVD).</p>
                        ...
                        <div class="bg-[#0d1117] rounded-xl border border-red-500/10 shadow-2xl overflow-hidden font-mono text-xs sm:text-sm shadow-[0_0_30px_rgba(0,0,0,0.5)]">
                            ...
                            <div class="p-5 space-y-3 text-zinc-300 h-72 overflow-y-auto custom-scrollbar">
                                ...
                                <p class="text-zinc-500">Starting Nmap 7.94 ( https://nmap.org )</p>
                                <div class="pl-4 mt-2">
                                    <p class="text-zinc-400 font-bold">PORT     STATE  SERVICE VERSION</p>
                                    <p class="text-zinc-300">22/tcp   <span class="text-emerald-400">open</span>   ssh     OpenSSH 8.2p1 Ubuntu 4ubuntu0.5</p>
                                    <p class="text-zinc-300">80/tcp   <span class="text-emerald-400">open</span>   http    <span class="text-red-400 font-bold bg-red-500/10 px-1 border border-red-500/30 rounded">Apache httpd 2.4.49</span></p>
                                </div>
                                ...
                                <div class="mt-4 border-t border-red-500/20 pt-4 pl-4 relative">
                                    ...
                                    <p class="text-rose-300">|   VULNERABLE: <span class="font-bold text-white">Path traversal and file disclosure vulnerability</span></p>
                                    <p class="text-zinc-400">|     State: VULNERABLE</p>
                                    <p class="text-zinc-400">|     IDs:  CVE:CVE-2021-41773</p>
                                    <p class="text-zinc-400">|     Risk factor: <span class="text-red-500 font-bold">High  CVSS: 7.5</span></p>
                                    <p class="text-zinc-400">|       A flaw was found in a change made to path normalization in Apache HTTP Server.</p>
                                </div>
                ...
                {:else if activeTab === 2}
                    <div transition:fade={{duration: 200}}>
                        ...
                        <p class="text-sm text-zinc-400 mb-6 max-w-3xl">Mitigating port scanning and 1-day exploit chaining requires severe attack surface reduction and intrusion prevention systems.</p>
...
<span class="text-zinc-500 italic"># 1. Deny all incoming traffic by default</span>
<span class="text-emerald-400">sudo</span> ufw default deny incoming

<span class="text-zinc-500 italic"># 2. Open strictly necessary ports</span>
<span class="text-emerald-400">sudo</span> ufw allow 443/tcp

<span class="text-zinc-500 italic"># 3. Restrict Admin services (SSH/22) to VPN IPs only</span>
<span class="text-emerald-400">sudo</span> ufw allow from <span class="text-orange-300">10.8.0.0/24</span> to any port 22

<span class="text-zinc-500 italic"># 4. Enable ufw rate-limiting (blocks brute-forcers)</span>
            ...
            <div class="bg-[#0A0C10] p-4 border-t border-red-500/10 flex items-center justify-between shrink-0 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.5)] z-20">
                ...
                <div class="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                    ...
                    {#if activeTab < tabs.length - 1}
                        <button 
                            onclick={nextTab}
                            class="flex items-center gap-1.5 px-6 py-2.5 bg-red-600 hover:bg-red-500 text-white text-sm font-semibold rounded-xl shadow-[0_0_20px_rgba(239,68,68,0.4)] transition-all transform hover:scale-105 active:scale-95 focus:ring-2 focus:ring-red-500/50 outline-none"
                        >
