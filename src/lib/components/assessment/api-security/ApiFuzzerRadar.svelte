...
<div class="border border-white/5 bg-black/40 p-6 rounded-xl backdrop-blur-xl h-full flex flex-col justify-center">
    ...
    <div class="space-y-5">
        {#each categories as c}
            <div>
                <div class="flex justify-between items-center mb-2">
                    <span class="text-sm font-medium {c.count > 0 ? (c.critical > 0 ? 'text-red-400' : 'text-rose-400') : 'text-zinc-500'} transition-colors">{c.name}</span>
                    <span class="text-xs font-mono {c.count > 0 ? 'text-white' : 'text-zinc-600'}">{c.count} Found</span>
                </div>
                <div class="h-2.5 w-full bg-white/5 rounded-full overflow-hidden shadow-inner">
                    <div 
        ...
        {/each}
        {#if totalVulns === 0}
            <div class="pt-4 text-center text-zinc-500 text-sm">
